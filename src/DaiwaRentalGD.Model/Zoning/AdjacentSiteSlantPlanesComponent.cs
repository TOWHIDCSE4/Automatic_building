...
{
    /// <summary>
    /// Represents the adjacent site slant planes of a site.
    /// </summary>
    [Serializable]
    public class AdjacentSiteSlantPlanesComponent : Component
    {
        #region Constructors

        public AdjacentSiteSlantPlanesComponent() : base()
        { }

        protected AdjacentSiteSlantPlanesComponent(
        ...
        ) : base(info, context)
        {
            ...
            _violations.AddRange(
                reader.GetValues<AdjacentSiteSlantPlanesViolation>(
                    nameof(Violations)
                )
            );
        }
        ...
        public double GetPropertyEdgeSlopeLength()
        {
            if (Site == null)
            {
                throw new InvalidOperationException(
            ...
            Vector<double> minVector = new DenseVector(
                new[]
                {
                    siteBbox.MinX,
                    siteBbox.MinY,
            ...
            Vector<double> maxVector = new DenseVector(
                new[]
                {
                    siteBbox.MaxX,
                    siteBbox.MaxY,
            ...
            double diagonal = (maxVector - minVector).L2Norm();

            double slopeLength =
                diagonal / Math.Cos(Math.Atan(SlopeAngleTangent));
        ...
        public AdjacentSiteSlantPlanesViolation GetViolation(Point point)
        {
            if (Site == null)
            {
                return new AdjacentSiteSlantPlanesViolation { Point = point };
            }
            ...
            foreach (int edgeIndex in Site.SiteComponent.PropertyEdgeIndices)
            {
                if (ViolatesPropertyEdgePlanes(edgeIndex, point))
                {
        ...
        public void UpdateViolations()
        {
            ...
            List<AdjacentSiteSlantPlanesViolation> violations =
                testPoints
                .Select(point => GetViolation(point))
                .Where(violation => violation.PropertyEdgeIndices.Any())
                .ToList();
        ...
        public IReadOnlyList<AdjacentSiteSlantPlanesViolation> Violations =>
            _violations;
        ...
        private readonly List<AdjacentSiteSlantPlanesViolation>
            _violations = new List<AdjacentSiteSlantPlanesViolation>();
